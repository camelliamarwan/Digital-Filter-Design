...
class MainWindow(QtWidgets.QMainWindow):
    ...
    #     filter_order = max(len(self.zeros_list), len(self.poles_list))
    #     start_index = max(0, len(self.mag_list) - (filter_order))
    #     to_be_filtered = self.mag_list[start_index : ]
        


    #     numerator, denominator = signal.zpk2tf(zeros_array, poles_array, 1)
    #     filtered_signal = signal.lfilter(numerator, denominator, to_be_filtered)

    #     self.filtered_signal.append(filtered_signal[len(filtered_signal)-1])
    #     # print('to be filtered:', to_be_filtered, 'filtered:',self.filtered_signal)
    #     # print(len(self.time_list))
    #     # print (len(self.filtered_signal))
    #     # print(filtered_signal[len(filtered_signal)-1])
    ...
    def on_press(self, event):
            ...
            if event.inaxes == self.canvas1.axes:
                ...
                if x is not None and y is not None:
                    ...
                    for primary_marker, conjugate_marker in self.zeros + self.poles:
                        ...
                        # print("zeros list", self.zeros_list)
        ...
        self.plot_frequency_response_mag(self.canvas_mag, self.frequancy_responce_graph, self.layout_mag)
        self.plot_frequency_response_phase(self.canvas_phase, self.phase_responce_graph, self.layout_phase, flag=False)
        # print(f"on press: {self.zeros_list}")
